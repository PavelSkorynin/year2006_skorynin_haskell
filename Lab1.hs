...
-- 1. Ноль - не натуральное число. Я не стал исправлять это,
-- но могу сделать, если необходимо. Из-за того, что ноль отнесён к
-- натуральным числам, возникает проблема с однозначным определением
-- целых чисел. В итоге в множестве целых чисел два нуля - положительный
-- и отрицательный.
-- 2. Не понял, какую "разумную" операцию должна выполнять функция
-- gfilter. Реализовал, как получилось. maybefilter' реализовал без
-- использования gfilter, так как не вижу в этом необходимости.
-- 3. Не придумал как реализовать maybe2either. А именно: не разобрался с
-- тем, какое значение есть у типа без конструктора.
---------------------------------------------
---------------------------------------------
---------------------------------------------
